...
## [#](https://linbudu.top/posts/2020/03/25/babel-class.html#基础-class-代码分析)基础 Class 代码分析
...
> 类似的常用内置 Symbol 值还有`Symbol.iterator`(在对对象进行`for...of`循环时, 会调用此方法) `Symbol.split`(`String.prototype.split(separator, limit)`等同于`separator[Symbol.split](this, limit)`) `Symbol.toPrimitive`(在对象被转换为原始类型值时调用,详见[Symbol.toPrimitive (opens new window)](https://es6.ruanyifeng.com/#docs/symbol#Symbol-toPrimitive))等
...
## [#](https://linbudu.top/posts/2020/03/25/babel-class.html#继承)继承
...
如果没接触过`Reflect`, 可以看一下[阮老师的文章 (opens new window)](https://es6.ruanyifeng.com/#docs/reflect), 这里使用的`Reflect.construct(target, args)`其实就等同于`new target(...args)`, 相当于一种不使用 new 来调用构造函数的方式.
...
### [#](https://linbudu.top/posts/2020/03/25/babel-class.html#详解)详解
...
[原文链接](https://linbudu.top/posts/2020/03/25/babel-class.html)
